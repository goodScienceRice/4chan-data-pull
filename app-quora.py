...
# Vantiq URL for posting JSON data
VANTIQ_URL = 'https://dev.vantiq.com/api/v1/resources/services/com.epigen.signal.ModelReturn/NewContent?token=O0QwbuIAViMVXAX_FGBbpAh5Rv-h6m77-tyuxOdvrW4='
    ...
    for i, q_box in enumerate(q_boxes, start=1):
        ...
        if s3_bucket:
            ...
            upload_file_to_s3(file_path, s3_bucket, s3_file_key)
             ...
             # Construct the full S3 URL (now including the merged_file.txt)
            s3_url = f"https://{s3_bucket}.s3.amazonaws.com/{s3_file_key}"
...
async def run(playwright: Playwright, topic: str, output_dir: str, s3_bucket: str = None, scroll_limit: int = 10) -> dict:
    ...
    if s3_bucket:
        ...
        #Now send the message to Vantiq
        send_message_to_vantiq(s3_bucket, s3_file_key)
...
def send_message_to_vantiq(bucket_name: str, s3_file_key: str) -> None:
    """
    Sends the S3 URL along with 'merged_file.txt' message to Vantiq.

    Args:
        bucket_name (str): The S3 bucket name.
        s3_file_key (str): The S3 object key.
    """
    ...
    payload = {
        "message": f"{s3_url}merged_file.txt"
    }

    # Vantiq API endpoint (replace with actual URL)
    vantiq_url = "https://dev.vantiq.com/api/v1/resources/services/com.epigen.signal.ModelReturn/NewContent?token=O0QwbuIAViMVXAX_FGBbpAh5Rv-h6m77-tyuxOdvrW4="

    # Send the payload to Vantiq
    response = requests.post(vantiq_url, json=payload)

    # Check the response
    if response.status_code == 200:
        print("Message sent successfully to Vantiq!")
    else:
        print(f"Failed to send message to Vantiq, status code: {response.status_code}")
